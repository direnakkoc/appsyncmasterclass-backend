...
describe('Given an authenticated userA', () => {
  let userA
  beforeAll(async () => {
  ...
  describe('When she sends a tweet', () => {
    ...
    describe('When she calls getTweets', () => {
      ...
      beforeAll(async () => {
        const result = await when.a_userA_calls_getTweets(userA, userA.userAname, 25)
      ...
      it('She cannot ask for more than 25 tweets in a page', async () => {
        await expect(when.a_userA_calls_getTweets(userA, userA.userAname, 26))
          .rejects
          .toMatchObject({
    ...
    describe('When she calls getMyTimeline', () => {
      let tweets, nextToken
      beforeAll(async () => {
        const result = await when.a_userA_calls_getMyTimeline(userA, 25)
        tweets = result.tweets
        nextToken = result.nextToken
      })
    ...
    describe('When she likes the tweet', () => {
      ...
      it('Should see this tweet when she calls getLikes', async () => {
        const { tweets, nextToken } = await when.a_user_calls_getLikes(userA, userA.userAname, 25)
      ...
      describe('When she unlikes the tweet', () => {
        beforeAll(async () => {
        ...
        it ('Should not see this tweet when she calls getLikes anymore', async () => {
          const { tweets, nextToken } = await when.a_user_calls_getLikes(userA, userA.userAname, 25)
    ...
    describe('When she retweets the tweet', () => {
    beforeAll(async () => {
      await when.a_userA_calls_retweet(userA, tweet.id)
    })

    it('Should see the retweet when she calls getTweets', async () => {
      const { tweets } = await when.a_userA_calls_getTweets(userA, userA.userAname, 25)
    ...
    it('Should not see the retweet when she calls getMyTimeline', async () => {
      const { tweets } = await when.a_userA_calls_getMyTimeline(userA, 25)
    ...
    describe('When she unretweets the tweet', () => {
      beforeAll(async () => {
    ...
    describe('Given another user, user B, sends a tweet', () => {
      ...
      describe("When user A retweets user B's tweet", () => {
        ...
        describe("When user A unretweets user B's tweet", () => {
          ...
          it('User A should not see the retweet when she calls getTweets anymore', async () => {
            const { tweets } = await when.a_user_calls_getTweets(userA, userA.username, 25)
          ...
          it('User A should not see the retweet when she calls getMyTimeline anymore', async () => {
            const { tweets } = await when.a_user_calls_getMyTimeline(userA, 25)
